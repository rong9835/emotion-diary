...
test.describe('회원가입 폼 기능 테스트', () => {
  ...
  test('1-1. 모든 필드가 비어있을 때 회원가입 버튼이 비활성화되어야 함', async ({ page }) => {
    const submitButton = page.locator('[data-testid="signup-submit-button"]');

    await expect(submitButton).toBeDisabled();
  });

  test('1-2. 모든 필드가 입력되면 회원가입 버튼이 활성화되어야 함', async ({ page }) => {
  ...
  test('1-3. 이메일에 @ 미포함 시 에러 메시지가 표시되어야 함', async ({ page }) => {
  ...
  test('1-4. 비밀번호가 영문+숫자 8자리 미만일 때 에러 메시지가 표시되어야 함', async ({ page }) => {
    const timestamp = Date.now();
  ...
  test('1-5. 비밀번호와 비밀번호 확인이 다를 때 에러 메시지가 표시되어야 함', async ({ page }) => {
    const timestamp = Date.now();

    await page.fill('[data-testid="signup-email-input"]', `test${timestamp}@example.com`);
    await page.fill('[data-testid="signup-password-input"]', 'Test1234');
    await page.fill('[data-testid="signup-password-confirm-input"]', 'Different1234');
    await page.fill('[data-testid="signup-name-input"]', '테스트');
  ...
  test('1-6. 이름이 1글자 미만일 때 에러 메시지가 표시되어야 함', async ({ page }) => {
    const timestamp = Date.now();

    await page.fill('[data-testid="signup-email-input"]', `test${timestamp}@example.com`);
    await page.fill('[data-testid="signup-password-input"]', 'Test1234');
    await page.fill('[data-testid="signup-password-confirm-input"]', 'Test1234');
    await page.fill('[data-testid="signup-name-input"]', ''); // 빈 이름
  ...
  test('2-1. 회원가입 성공 시 가입완료 모달이 표시되어야 함', async ({ page }) => {
    const timestamp = Date.now();
    ...
    const successModal = page.locator('[data-modal-component="true"]').filter({ hasText: '가입이 완료되었습니다' });
    await expect(successModal).toBeVisible({ timeout: 1500 });
  });

  test('2-2. 가입완료 모달에서 확인 클릭 시 로그인 페이지로 이동해야 함', async ({ page }) => {
    const timestamp = Date.now();
  ...
  test('2-3. 같은 성공 모달은 한 번만 표시되어야 함', async ({ page }) => {
    ...
    await page.fill('[data-testid="signup-email-input"]', `test${timestamp2}@example.com`);
    await page.fill('[data-testid="signup-password-input"]', 'Test1234');
    await page.fill('[data-testid="signup-password-confirm-input"]', 'Test1234');
  ...
  test('3-1. 이메일 중복 시 가입실패 모달이 표시되어야 함', async ({ page }) => {
    ...
    await page.fill('[data-testid="signup-email-input"]', `duplicate${timestamp}@example.com`);
    await page.fill('[data-testid="signup-password-input"]', 'Test1234');
    await page.fill('[data-testid="signup-password-confirm-input"]', 'Test1234');
    await page.fill('[data-testid="signup-name-input"]', '테스트사용자');
    ...
    // 실패 모달 대기
    const failureModal = page.locator('[data-modal-component="true"]').filter({ hasText: '회원가입에 실패했습니다' });
    await expect(failureModal).toBeVisible({ timeout: 1500 });
  });

  test('3-2. 가입실패 모달에서 확인 클릭 시 모달만 닫히고 페이지는 유지되어야 함', async ({ page }) => {
    // GraphQL API를 mocking하여 에러 응답 반환
    await page.route('**/graphql', async (route) => {
      const postData = route.request().postData();
    ...
    await page.fill('[data-testid="signup-email-input"]', `error${timestamp}@example.com`);
    await page.fill('[data-testid="signup-password-input"]', 'Test1234');
    await page.fill('[data-testid="signup-password-confirm-input"]', 'Test1234');
    await page.fill('[data-testid="signup-name-input"]', '테스트사용자');
