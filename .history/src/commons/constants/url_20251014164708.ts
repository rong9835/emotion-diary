 ...
 * @returns UI 컴포넌트 노출 설정
 */
export const getUIVisibilityByPath = (
  currentPath: string
): UIComponentVisibility | null => {
  const config = getPageConfigByPath(currentPath);
  return config?.uiVisibility || null;
 ...
 * 공개 페이지 키 배열
 */
export const PUBLIC_PAGE_KEYS = ALL_PAGE_KEYS.filter(
  (key) => PAGE_CONFIGS[key].accessPermission === AccessPermission.PUBLIC
);

/**
 * 회원 전용 페이지 키 배열
 */
export const MEMBER_ONLY_PAGE_KEYS = ALL_PAGE_KEYS.filter(
  (key) => PAGE_CONFIGS[key].accessPermission === AccessPermission.MEMBER_ONLY
);

/**
 * 다이나믹 라우팅 페이지 키 배열
 */
export const DYNAMIC_PAGE_KEYS = ALL_PAGE_KEYS.filter(
  (key) => PAGE_CONFIGS[key].isDynamic
);
